...
#oder data by amp
sorter = matrix(0,length(bs_C[,1]),1)
for (step in 1:length(bs_C[,1])) {
  sorter[step] = max(bs_C[step , 20 : 25 ] ) - min(bs_C[step , 20 : 25 ] )
}

bs_C_amp = bs_C[ order(sorter, decreasing = T), ]
...
result_amp = data.frame(matrix(0,40,6))
corr_amp_all = array(0, dim=c(10,10,40))
corr_amp_rt = array(0, dim=c(10,10,40))
corr_amp_nort = array(0, dim=c(10,10,40))
...
corr_amp_all[,,amount] = cor(genes_check[, 6:15], method = "spearman")
corr_amp_rt[,,amount] = cor(genes_rb_rt[, 6:15], method = "spearman")
corr_amp_nort[,,amount] = cor(genes_rb_nort[, 6:15], method = "spearman")
...
result_amp[amount,] = rhyth_numbr

}
####################################################################


result_amp[,4] = result_amp[,2]-result_amp[,1]
result_amp[,7] = result_amp[,4]/result_amp[,2]
result_amp[,8] = result_amp[,1]/result_amp[,2]

colnames(result_amp) = c("rb+rt", "rb", "rt", "rb-rt", "rhyth%", "top", "rb-rt/rb", "rt+rb/rb")

par(mfrow=c(1,3))
plot(result_amp[,6],result_amp[,5])
plot(result_amp[,6],result_amp[,8])
plot(result_amp[,6],result_amp[,7])
